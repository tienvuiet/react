...
class Library {
    ...
    updateBookById(id: number, title: string, author: string): string {
        let found = false
        this.book.forEach((b) => {
            if (b.getId() === id) {
    ...
    deleteBookById(id: number):string{
        const lengthBookHT = this.book.length
        this.book = this.book.filter(b=>b.getId()!==id)
        return lengthBookHT>this.book.length?`Da xoa`:`Khong xoa duoc`
    }
    showBook() {
...
library. updateBookById(2, "Chinh sua id", "chinh sua")

console.log(library.deleteBookById(4));
library.showBook()